        ...
        <div class="col-md-8">
            <div class="main-card mb-3 card">
                <div class="card-body">
                    ...
                    {{ html()->form('post', route('admin.stock-receive.add', 'step=2&add=true'))->attribute('enctype', 'multipart/form-data')->open() }}
                    ...
                    <div class="form-row">
                            ...
                            <div class="position-relative form-group">
                                ...
                                <select id="item_id" name="item_id" class="form-control">
                                    @foreach($items as $item)
                                        <option value="{{ $item->id }}" data-price="{{ $item->price_supplier }}">{{ $item->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        <div class="col-md-2">
                            <div class="position-relative form-group">
                                <label for="price_supplier" class="">Actual(RM)</label>
                                {{ html()->text('price_supplier')->class('form-control')->readonly() }}
                            </div>
                        </div>
                        <div class="col-md-2">
                            <div class="position-relative form-group">
                    ...
                    <a href="{{ route('admin.stock-receive.add', 'step=1') }}" onclick="return confirm('Are you sure? All saved product will be deleted.')" class="btn btn-icon btn-lg btn-warning"><i class="fa fa-arrow-left btn-icon-wrapper"></i> Back</a>
                    <div class="float-right">
                        <button type="submit" class="btn btn-lg btn-success btn-icon"><i class="fa fa-plus btn-icon-wrapper"></i> Add Product</button>
                        <a href="{{ route('admin.stock-receive.add', 'step=3') }}" class="btn btn-lg btn-primary btn-icon"><i class="fa fa-arrow-right btn-icon-wrapper"></i> Next</a>
                    </div>
                    {{ html()->form()->close() }}
                </div>
            </div>
                ...
                <div class="card-body">
                    ...
                    <table class="mb-0 table">
                        ...
                        @if(isset(Session::get('receive_product')['product']))
                            ...
                            <tr>
                                <td><a href="{{ route('admin.stock-receive.remove-from-list', $key) }}" onclick="return confirm('Are you sure want to remove this product from list?')"><i class="fa fa-times text-danger"></i> </a>{{ $item['code'] }}</td>
...
<script type="text/javascript">
    ...
    $('#item_id').on('change', function() {
        ...
        $('#price_supplier').val($(this).find(':selected').data('price'));
